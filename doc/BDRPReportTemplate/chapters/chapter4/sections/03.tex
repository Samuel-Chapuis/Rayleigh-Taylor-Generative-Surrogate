...
configuration, initializes the SGM and U-Net, launches the training loop when
...
normalizes them in the interval $[-1,1]$. The \texttt{SGM} module implements a
continuous VP-SDE with affine $\beta(t)$, exact marginal noising, score
reconstruction, and probability-flow ODE sampling using Heun by default. The \texttt{UNet} module
provides the time-conditioned convolutional network used to predict either the
Gaussian perturbation or, in the current default configuration, the
$\mathbf{v}$-parameterization. The \texttt{training\_loop} module contains the
training logic: for each batch, a continuous diffusion time is sampled,
Gaussian noise is added using the exact VP marginal, the U-Net predicts the
chosen SGM target, and the parameters are updated by minimizing the mean
squared error between true and predicted targets. Validation loss is used to
select and save the best model checkpoint.
...
the default $\mathbf{v}$-parameterization used in \texttt{SGM.py}, the target is
\[
\mathbf{v}
=
...
\FOR{each training epoch}
    \FOR{each batch $x_0$}
        \STATE Sample a continuous time $t\in[0,1]$ and Gaussian noise $\epsilon$.
...
from a Gaussian prior at $t=1$ and integrating the deterministic
...
\STATE Sample $x_1\sim\mathcal{N}(0,I)$ with the target image shape.
