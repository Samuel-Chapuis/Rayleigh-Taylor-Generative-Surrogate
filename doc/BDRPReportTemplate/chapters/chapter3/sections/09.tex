...
\begin{compactproof}[title={Example: closed-form noising law}]
\proofstep{Idea} Starting from the discrete Gaussian transition, one repeatedly
substitutes the previous state and uses the stability of Gaussian variables under
independent affine combinations. For two consecutive steps,
\[
\mathbf{x}_t
=
\sqrt{\alpha_t\alpha_{t-1}}\,\mathbf{x}_{t-2}
+\sqrt{\alpha_t(1-\alpha_{t-1})}\,\boldsymbol{\epsilon}_{t-1}
+\sqrt{1-\alpha_t}\,\boldsymbol{\epsilon}_{t}.
\]
Since the two noise terms are independent and centered, their sum is again
Gaussian with covariance
\[
\alpha_t(1-\alpha_{t-1})\mathbf{I}
+(1-\alpha_t)\mathbf{I}
=
(1-\alpha_t\alpha_{t-1})\mathbf{I}.
\]
Iterating this argument yields the compact marginal expression above with
\(\bar{\alpha}_t=\prod_{s=1}^{t}\alpha_s\). \qedcompact
\end{compactproof}
