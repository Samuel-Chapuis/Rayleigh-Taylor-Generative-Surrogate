...
The transition score is therefore known exactly for every pair
$(\mathbf{x}_0,\mathbf{x}_t)$ generated by the forward process. However, it
must be distinguished from the marginal score required in the reverse-time
SDE. Indeed,
\begin{equation}
    \nabla_{\mathbf{x}_t}\log p_t(\mathbf{x}_t)
    =
    \mathbb{E}\!\left[
        ...
        \mathbf{x}_t
    \right].
    \label{eq:marginal_score_conditional_noise}
\end{equation}
For a fixed noisy observation $\mathbf{x}_t$, the injected noise
$\boldsymbol{\epsilon}$ is not uniquely determined because several clean
samples $\mathbf{x}_0$ may lead to the same value of $\mathbf{x}_t$. The
marginal score is consequently the conditional mean of the transition scores
compatible with the observed noisy sample.

This identity makes the score learnable from samples. Recall that, for any
square-integrable random variable $\mathbf{Y}$, the conditional expectation
$\mathbb{E}[\mathbf{Y}\mid\mathbf{X}]$ is the function of $\mathbf{X}$ that
minimizes the mean squared prediction error. Applying this property with
\begin{equation*}
    \mathbf{Y}
    =
    \nabla_{\mathbf{x}_t}
...
\begin{equation}
    \nabla_{\mathbf{x}_t}\log p_t(\mathbf{x}_t)
    =
    \underset{\mathbf{f}}{\operatorname{argmin}}\;
    \mathbb{E}_{p_{0,t}}
    \left[
        \left\|
        ...
        \right\|_2^2
    \right].
    \label{eq:score_conditional_projection}
\end{equation}
The expectation in equation~\eqref{eq:score_conditional_projection} is taken
...
\begin{equation*}
    ...
    p_{t\mid 0}(\mathbf{x}_t\mid\mathbf{x}_0),
\end{equation*}
from which samples are directly available: one first draws a clean field
$\mathbf{x}_0$ from the dataset, then a diffusion time $t$, and finally a
Gaussian perturbation $\boldsymbol{\epsilon}$ used to construct
$\mathbf{x}_t=\alpha_t\mathbf{x}_0+\sigma_t\boldsymbol{\epsilon}$.

A neural network $\mathbf{s}_{\theta}(\mathbf{x},t)$ is used to approximate the
score simultaneously at all diffusion times,
\begin{equation}
    \mathbf{s}_{\theta}(\mathbf{x},t)
    \approx
    \nabla_{\mathbf{x}}\log p_t(\mathbf{x}),
    \qquad
    \mathbf{x}\in\mathbb{R}^{d},\quad t\in[0,T].
\end{equation}
Introducing a non-negative time-dependent weight $\lambda(t)$, denoising score
matching therefore leads to the objective
\begin{equation}
\begin{aligned}
...
\end{aligned}
\end{equation}
The weighting function $\lambda(t)$ balances the contribution of the different
noise levels. This is important because the magnitude of the transition score
scales as $1/\sigma_t$: without an appropriate weighting, small-noise samples
may dominate the loss.

In practice, DDPM implementations generally use an equivalent
noise-prediction parameterization. Instead of predicting the score directly,
the network $\boldsymbol{\epsilon}_{\theta}(\mathbf{x}_t,t)$ predicts the
Gaussian perturbation used to construct the noisy sample. The two
...
\begin{equation}
    ...
    \label{eq:score_noise_parameterization}
\end{equation}
Substituting equation~\eqref{eq:score_noise_parameterization} into the
score-matching objective gives
\begin{align}
    \mathcal{L}_{\mathrm{DSM}}(\theta)
    &=
    \mathbb{E}_{t,\mathbf{x}_0,\boldsymbol{\epsilon}}
    \left[
        ...
        \right\|_2^2
    \right].
    \label{eq:dsm_noise_weighted}
\end{align}
Thus, score prediction and noise prediction differ only by a known rescaling
and by the associated time weighting. Choosing
$\lambda(t)=\sigma_t^2$ yields the commonly used simplified DDPM loss
introduced by \citet{Ho2020DDPM},
\begin{equation}
    \boxed{
...
\end{equation}
% At the population optimum of this quadratic loss, the network predicts the
% conditional mean of the noise,
% \begin{equation}
%     \boldsymbol{\epsilon}_{\theta^*}(\mathbf{x}_t,t)
%     =
%     \mathbb{E}\!\left[
...
%     \right],
% \end{equation}
% and equation~\eqref{eq:marginal_score_conditional_noise} becomes
% \begin{equation}
%     \nabla_{\mathbf{x}_t}\log p_t(\mathbf{x}_t)
%     =
%     -\frac{1}{\sigma_t}
...
%         \sigma_t
%     }.
%     \label{eq:learned_score_from_noise}
% \end{equation}
% This relation is a particular form of Tweedie's formula: predicting the noise,
% predicting the clean sample, and estimating the score are equivalent regression
% problems up to deterministic time-dependent transformations.

% Once training is complete, the estimated score is inserted into the
% reverse-time SDE. Starting from a sample drawn from the terminal Gaussian prior,
% \begin{equation}
%     \mathbf{x}_T
%     \sim
%     \mathcal{N}(\mathbf{0},\mathbf{I}),
% \end{equation}
% the reverse dynamics is integrated from $T$ to $0$,
% \begin{equation}
%     d\mathbf{x}
%     =
%     \left[
%         \mathbf{f}(\mathbf{x},t)
%         -
%         g(t)^2\mathbf{s}_{\theta}(\mathbf{x},t)
%     \right]dt
%     +
%     g(t)d\bar{\mathbf{w}}.
%     \label{eq:learned_reverse_sde}
% \end{equation}
% For the VP process, the score term progressively compensates for the diffusion
% of probability mass produced by the forward process. At large noise levels,
% the marginal distribution is close to a centered Gaussian and the score mainly
% points toward its central high-probability region. At smaller noise levels, the
% score field becomes increasingly structured and separates trajectories toward
% the different modes of the data distribution.
...
let $ \tau = T - t$ and so $ \mathbf{y}_t = \mathbf{x}_{T-t} $ and $ q_t(\mathbf{y}) = p_{T-t}(\mathbf{y}) $. The reverse process is then written with a time variable increasing from $ t = 0 $ to $ t = T $:
\begin{equation}
    d\mathbf{y}
    =
    \left[
        -\mathbf{f}(\mathbf{y},T-t)
        +g(T-t)^2
        \nabla_{\mathbf{y}}\log p_{T-t}(\mathbf{y})
    \right]dt
    +
    g(T-t)d\overline{\mathbf{w}}.
