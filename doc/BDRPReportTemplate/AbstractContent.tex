...
Rayleigh--Taylor instability (RTI), but their computational cost limits the number of
...
compare two diffusion-based strategies for the generation of two-dimensional
Rayleigh--Taylor instability fields obtained from Direct Numerical Simulations (DNS): a classical diffusion
...
RTI and emphasizes its strongly non-linear and
...
flow structures. The first generative approach is a baseline Score Base Generative Model (SGM) trained directly in image space on preprocessed
density fields, using physically consistent data augmentations based on horizontal
...
distances, including Fréchet Inception Distance (FID) and a domain-adapted physical
PhyFID, as well as physically
...
the generated fields is 60 time closer to the validation set than pure noise. The physical metrics show
...
RTI. While classical diffusion currently gives the
